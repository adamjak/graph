...
static struct graph graph_init(int s)
{
    struct graph g = {
        .size = s,
        .nei = calloc(s*3, sizeof(int)),
        .deg = calloc(s, sizeof(short)),
        .hid = calloc(s, sizeof(int)),
        .col = NULL
    };
    return g;
}
...
{
    assert(!g->col);
    g->col = calloc(g->size * 3, sizeof(int));
}
...
{
    struct swtch *sw = calloc((g->size + 4)/4, sizeof(sw[0]));
    int *marked = calloc(g->size, sizeof(int));
    int found = 0;
    for (int i = 0; i < g->size; i++) {
...
static void count_cycles(struct graph *g, int *count, int *length)
{
    *count = 0;
    *length = 0;
    int *visit = malloc(g->size * sizeof(int));
    for (int i = 0; i < g->size; i++)
        visit[i] = 0;
    for (int i = 0; i < g->size; i++) {
...
int kowalik_colourise(char *mat, int size)
{
    //test_crossing();
    ...
    if (ret) {
        //showgraph(&g, &g);
//        if (!verify(&g)) {
//            fprintf(stderr, "Generated coloring is invalid\n");
//            showgraph(&g, &g);
//            abort();
//        }
        for (int i = 0; i < size; i++) {
